...
namespace BrownGirlsCodeUpdated.Models
{
    public class Class2
    {
        SqlConnection con;
        ...
        public bool adminVerify(string username, string pswd)
        {
            bool ver = false;
            int i = 0;
        ...
        public bool userVerify(string username, string pswd, string isactive)
        {
            bool ver = false;
            int i = 0;
            //   ver = checkavalaibility(Enrollment);
            try
            {
                ...
                if (con != null) { con.Close(); }
                con.Open();
                string querry2 = @"SELECT Password,isActive FROM tbl_Registration
                WHERE UserName = '" + username + "'";
                cmd = new SqlCommand(querry2, con);
                SqlDataReader reader2 = cmd.ExecuteReader();
                string pswds = "*";
                ...
                while (reader2.Read())
                {
                    pswds = reader2["Password"].ToString();
                    act = reader2["isActive"].ToString();
                    i++;
                }
                reader2.Close();
                if (i != 0)
        ...
        public bool InsertComments(string name, string comments)
        {
            if (con != null) { con.Close(); }
            con.Open();
            string query = "insert into designproficien.Comments(name,comments)VALUES (@name, @comments)";
        ...
        public bool InsertBlogs(string Blog_Title, string Post_By, string Post_Date, string Blog_Type, byte[] Image_Link, int Status, string Video_Link, string description)
        {
            if (con != null) { con.Close(); }
            con.Open();
            string query = "insert into designproficien.Blogs(Blog_title ,Post_by ,Post_date ,Blog_type,Image_link,Status,Video_link,Description)VALUES (@Blog_title, @Post_by, @Post_date, @Blog_type,@Image_link,@Status,@Video_link,@Description)";
